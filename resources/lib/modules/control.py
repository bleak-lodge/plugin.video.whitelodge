...
bookmarksFile = os.path.join(dataPath, 'bookmarks.3.db')

mylistsFile = os.path.join(dataPath, 'mylists.db')
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(resource.images.whitelodge.artwork)'):
        return os.path.join(xbmcaddon.Addon('resource.images.whitelodge.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(resource.images.whitelodge.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://resource.images.whitelodge.artwork)')
...
def clean_settings(info=True):
    ...
    try:
        ...
        add_ons = ['plugin.video.whitelodge']
